...
export function UploadStep({ copy, language, onImageLoaded }: UploadStepProps) {
  ...
  async function handleFile(file: File | undefined) {
    ...
    if (!validation.ok) {
      setError(getErrorMessage(language, validation.code));
      if (inputRef.current) {
    ...
    try {
      ...
      setError(getErrorMessage(language, getErrorCode(loadError, "image_load_failed")));
    } finally {
      setIsLoading(false);
    }
  }
  ...
  return (
    <section className="flex flex-1 flex-col justify-center gap-6 py-8">
      <div className="max-w-xl space-y-2">
        ...
        <p className="text-sm leading-6 text-zinc-600">{copy.upload.empty}</p>
      </div>
      ...
      >
        ...
        <span className="mt-2 inline-flex min-h-10 items-center justify-center bg-zinc-950 px-4 text-sm font-medium text-white">
          {isLoading ? copy.upload.loading : copy.upload.choose}
        </span>
        <input
          ref={inputRef}
          id="pixelbead-upload"
